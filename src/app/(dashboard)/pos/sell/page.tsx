...
const HOLD_KEY = 'erp_pos_sell_holds_v1';
...
/* ──────────────────────────────────────────────
   الصفحة الرئيسية — شاشة كاشير مستقلة
   ────────────────────────────────────────────── */
...
export default function POSSellPage() {
  ...
  const [draftId] = useState(() => `POS-${crypto.randomUUID().slice(0, 8).toUpperCase()}`);
  ...
  /* ── وسائل الدفع والإعدادات من الملف ── */
  ...
  /* ════════════════════════════════════════════
     العرض — شاشة كاملة للكاشير
     ════════════════════════════════════════════ */
  return (
    <div dir="rtl" className="flex flex-col h-screen -m-4 md:-m-6 lg:-m-8 overflow-hidden">
      {/* ── شريط العنوان ── */}
      <PosSellHeader
        draftId={draftId}
        ...
        <Tabs defaultValue="products" className="flex flex-1 flex-col min-h-0 min-w-0">
          <TabsList className="h-auto w-full flex-wrap justify-start gap-1 bg-muted/40 p-1 mx-3 my-2 rounded-xl shrink-0">
            ...
            >
              ...
              السلة
              {cart.length > 0 && (
                <span className="inline-flex items-center justify-center min-w-[1.1rem] h-4 rounded-full bg-primary text-[9px] text-primary-foreground px-1">
                  {totalItems}
                </span>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="products" className="flex-1 min-h-0 overflow-hidden mt-0 px-1">
            <PosSellProductColumn {...productPaneProps} compactGrid />
          </TabsContent>
          <TabsContent value="cart" className="flex-1 min-h-0 overflow-hidden mt-0">
            <PosSellCheckoutColumn {...checkoutProps} showReturnBar />
          </TabsContent>
          <TabsContent value="receipt" className="flex-1 min-h-0 overflow-y-auto mt-0 p-3">
            <PosSellSettingsPane
            ...
            />
            {lastReceipt && (
              <div className="mt-4 p-4 border rounded-xl bg-card space-y-2">
                <p className="text-sm font-semibold">آخر إيصال</p>
                <p className="text-xs text-muted-foreground font-mono">{lastReceipt.invoiceName}</p>
                <p className="text-lg font-bold">{formatCurrency(lastReceipt.total)}</p>
                <div className="flex gap-2 flex-wrap">
                  <Button type="button" size="sm" variant="outline" onClick={() => printBrowserReceipt(lastReceipt)}>
                    طباعة
                  </Button>
                  <Button type="button" size="sm" variant="outline" onClick={() => previewBrowserReceipt(lastReceipt)}>
                    معاينة
                  </Button>
                  {canUseWebSerialPrint() && (
                    <Button type="button" size="sm" variant="outline" onClick={() => void printSerialSimple(lastReceipt)}>
                      طابعة حرارية
                    </Button>
                  )}
                </div>
