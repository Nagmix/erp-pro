...
type TransactionType = 'all' | 'Sales Invoice' | 'Payment Entry' | 'Journal Entry' | 'Credit Note';
...
const TRANSACTION_TYPE_OPTIONS: { value: TransactionType; label: string }[] = [
  ...
  { value: 'Payment Entry', label: 'سند قبض' },
...
function getTransactionTypeLabel(voucherType: string): string {
  switch (voucherType) {
    ...
    case 'Payment Entry': return 'سند قبض';
    case 'Journal Entry': return 'قيد يومية';
    case 'Purchase Invoice': return 'فاتورة مشتريات';
    default:
      if (voucherType.includes('Credit Note') || voucherType.includes('Return')) return 'إشعار دائن';
      return voucherType;
  }
}
...
function getTransactionTypeBadgeColor(voucherType: string): string {
  switch (voucherType) {
    case 'Sales Invoice': return 'bg-amber-100/80 dark:bg-amber-950/30 text-amber-800 dark:text-amber-200';
    ...
    default:
      if (voucherType.includes('Credit Note') || voucherType.includes('Return'))
        return 'bg-purple-100/80 dark:bg-purple-950/30 text-purple-800 dark:text-purple-200';
...
function calculateAging(glEntries: GLEntry[], asOfDate: string): AgingBucket {
  ...
  for (const entry of glEntries) {
    const outstanding = (entry.debit || 0) - (entry.credit || 0);
...
export default function CustomerAccountStatementPage() {
  ...
  const [footerText, setFooterText] = useState('توقيع العميل: ........................');
  ...
  // --- Customer Info ---
  const { data: customerDoc } = useDoc<CustomerRecord>('Customer', selectedCustomer, {
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (transactionFilter === 'Credit Note') {
      return glEntries.filter(
        (e) => e.voucher_type.includes('Credit Note') || e.voucher_type.includes('Return')
      );
    }
  ...
  const summary = useMemo(() => {
    const totalDebit = statementRows.reduce((sum, e) => sum + (e.debit || 0), 0);
    const totalCredit = statementRows.reduce((sum, e) => sum + (e.credit || 0), 0);
    const netBalance = totalDebit - totalCredit;

    // Overdue: debit entries where posting_date + terms > today (simplified)
    ...
    const overdueAmount = statementRows.reduce((sum, e) => {
      if (e.debit > 0) {
        ...
        if (diffDays > 30) return sum + e.debit;
      }
      return sum;
    }, 0);

    return { totalDebit, totalCredit, netBalance, overdueAmount };
  }, [statementRows]);
  ...
  const handleEmail = useCallback(() => {
    const email = customerDoc?.email_id;
    if (!email) {
      toast.error('لا يوجد بريد إلكتروني مسجل لهذا العميل');
      return;
    }
    const subject = encodeURIComponent(`كشف حساب - ${customerDoc?.customer_name || selectedCustomer}`);
    const body = encodeURIComponent(
      `السلام عليكم،\n\nمرفق كشف حسابكم بتاريخ ${dateRange.from} إلى ${dateRange.to}.\n\nالرصيد المستحق: ${formatCurrency(endingBalance, 'YER')}\n\nمع تحياتنا`
    );
    window.open(`mailto:${email}?subject=${subject}&body=${body}`);
    toast.success('تم فتح برنامج البريد لإرسال كشف الحساب');
  }, [customerDoc, selectedCustomer, dateRange.from, dateRange.to, endingBalance]);
  ...
  return (
    <div className="erp-page-enter space-y-5" dir="rtl">
      <PageHeader
        ...
        description="عرض تفصيلي لمعاملات العميل المالية مع أرصدة تراكمية وتوزيع أعمار الذمم."
        ...
        actions={
          <div className="flex gap-2 print:hidden">
            ...
            {statementRows.length > 0 && (
              <ExportButton
                data={exportData}
                filename={`كشف حساب ${customerDoc?.customer_name || selectedCustomer}`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Button
                ...
                disabled={!selectedCustomer || loadingEntries}
              >
                {loadingEntries ? (
          ...
          <CardContent className="p-4">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold">{customerDoc.customer_name}</span>
                <Badge variant="outline" className="text-[10px]">{selectedCustomer}</Badge>
              </div>
              {customerAccount && (
                <div className="flex items-center gap-1 text-muted-foreground">
                  <Calculator className="h-3.5 w-3.5" />
                  <span className="text-xs">الحساب: {customerAccount}</span>
                </div>
              )}
              {customerDoc.email_id && (
                <div className="flex items-center gap-1 text-muted-foreground">
                  <Mail className="h-3.5 w-3.5" />
                  <span className="text-xs">{customerDoc.email_id}</span>
                </div>
              )}
              <div className="flex items-center gap-1 text-muted-foreground">
                <CalendarClock className="h-3.5 w-3.5" />
      ...
      {loadingEntries && (
        <div className="flex items-center gap-2 py-12 justify-center text-muted-foreground">
          ...
          <span className="text-sm">جاري تحميل كشف الحساب...</span>
        </div>
      )}
      ...
      {!loadingEntries && !entriesError && !selectedCustomer && (
        <EmptyState
          title="اختر العميل"
      ...
      {!loadingEntries && !entriesError && selectedCustomer && glEntries.length === 0 && (
        <EmptyState
          title="لا توجد معاملات"
          description="لا توجد معاملات مالية لهذا العميل ضمن الفترة المحددة."
        />
      )}
      ...
      {statementRows.length > 0 && !loadingEntries && (
        <div ref={printRef}>
          ...
          <div className="hidden print:block print:mb-6">
            <h1 className="text-xl font-bold text-center mb-2">كشف حساب عميل</h1>
            <div className="flex justify-between text-sm">
              <span>العميل: {customerDoc?.customer_name || selectedCustomer}</span>
              <span>الفترة: {dateRange.from} إلى {dateRange.to}</span>
            </div>
            {customerAccount && (
              <div className="text-sm text-muted-foreground">الحساب: {customerAccount}</div>
            )}
          </div>
                ...
                <Table>
                  <TableHeader className="sticky top-0 z-20 bg-muted/90 backdrop-blur">
                    <TableRow className="hover:bg-muted/60 border-b border-border/40">
                      ...
                      <TableHead className="text-xs font-semibold text-start" dir="ltr">مدين</TableHead>
                      <TableHead className="text-xs font-semibold text-start" dir="ltr">دائن</TableHead>
            ...
            <Card className="border-border/40">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  <span className="text-sm text-muted-foreground">إجمالي المدين (على العميل)</span>
                  <span className="text-sm font-semibold text-red-600 dark:text-red-400" dir="ltr">
                    {formatCurrency(summary.totalDebit, 'YER')}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  <span className="text-sm text-muted-foreground">إجمالي الدائن (لصالح العميل)</span>
                ...
                <div className="flex justify-between items-center py-2 border-b border-border/20">
                  ...
                  <span className={cn(
                    'text-sm font-bold',
                    summary.netBalance > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'
                  )} dir="ltr">
                    {formatCurrency(summary.netBalance, 'YER')}
                  </span>
                </div>
          ...
          <Card className="border-border/40 mt-4">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">توقيع العميل</Label>
