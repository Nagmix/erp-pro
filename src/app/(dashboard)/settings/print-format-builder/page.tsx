      ...
      {expanded && (
        <div className="px-3 pb-2 pt-1 border-t border-border/30 space-y-2">
          <div className="flex items-center gap-3">
            <Label className="text-[10px] min-w-[40px]">المحاذاة</Label>
            <Select
          ...
          <div className="flex items-center gap-3">
            <Label className="text-[10px] min-w-[40px]">العرض</Label>
            <Input
              type="number"
              min={10}
              max={100}
...
function PreviewPanel({ sections, formatName }: { sections: PrintFormatSection[]; formatName: string }) {
  ...
  return (
    <div className="bg-white text-gray-900 rounded-lg border border-border/30 p-5 min-h-[300px] shadow-inner" dir="rtl">
      <div className="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
        <div>
          <h3 className="text-sm font-bold text-gray-800">{formatName || 'تنسيق طباعة'}</h3>
          <p className="text-[10px] text-gray-500">معاينة تقريبية — قد تختلف عن المخرج النهائي</p>
        </div>
          ...
          {visibleSections.map((section) => {
            ...
            return (
              <div key={section.id} className="space-y-2">
                <h4 className="text-xs font-semibold text-gray-600 border-b border-gray-100 pb-1">
                  {section.label}
                </h4>
                <div className="grid grid-cols-2 gap-x-6 gap-y-1.5">
                  {visibleFields.map((field) => (
                    <div key={field.fieldname} className="flex items-baseline gap-1.5">
                      <span className="text-[10px] text-gray-500 min-w-[80px]">{field.label}:</span>
                      <span
                        className={cn(
                          'text-[10px] text-gray-800 bg-gray-50 rounded px-1.5 py-0.5',
                          field.fieldtype === 'Currency' && 'font-mono',
                          field.fieldtype === 'Int' && 'font-mono',
                          field.fieldtype === 'Float' && 'font-mono',
                          field.fieldtype === 'Date' && 'font-mono text-gray-600',
                        )}
                        style={{
                    ...
                    <div className="space-y-3">
                      {groupedAvailable.map((g) => (
                        <div key={g.group} className="space-y-1">
                          <p className="text-[10px] font-semibold text-muted-foreground">{g.group}</p>
                          {g.fields.map((f) => (
                            <div
                              key={f.fieldname}
                              className="flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-muted/60 transition-colors cursor-pointer group"
                            >
                              <span className="flex-1 text-[10px] truncate">{f.label}</span>
              ...
              <CardContent className="p-4 pt-2 space-y-3">
                {sections.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                    ...
                    <p className="text-[10px]">أضف قسماً جديداً أو اختر تنسيقاً يحتوي على بيانات</p>
                  </div>
                ) : (
