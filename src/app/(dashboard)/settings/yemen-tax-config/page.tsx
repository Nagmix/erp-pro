 ...
 <div dir="rtl" className="erp-page-enter space-y-5">
  ...
  <TabsContent value="rates" className="space-y-4">
   ...
   <CardContent className="p-5 space-y-4">
    ...
    <div className="mt-0.5 rounded-lg bg-chart-2/15 p-2.5">
    ...
    <div className="max-h-96 overflow-y-auto custom-scrollbar">
     ...
     return (
      ...
      <div className="flex items-center gap-2">
       <Badge variant="outline" className={`text-[10px] border-0 ${isActive ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
       {isActive ? 'نشط' : 'معطّل'}
       </Badge>
       <Switch
       checked={isActive}
  ...
  <TabsContent value="filing" className="space-y-4">
   ...
   <Card className={netTaxPayable >= 0 ? 'border-destructive/20' : 'border-primary/20'}>
    <CardContent className="p-5 space-y-2">
    <div className={`flex items-center gap-2 ${netTaxPayable >= 0 ? 'text-destructive' : 'text-chart-3'}`}>
   ...
   <CardContent>
    ...
    <div className="space-y-2">
     ...
     <span className="font-semibold text-chart-3">+{formatCurrency(salesTaxThisMonth)}</span>
     </div>
     <div className="flex items-center justify-between text-sm">
     <span className="text-muted-foreground">ضريبة المشتريات المدفوعة (مدخلات)</span>
     <span className="font-semibold text-chart-2">-{formatCurrency(purchaseTaxThisMonth)}</span>
     </div>
     <Separator />
     <div className="flex items-center justify-between text-sm font-bold">
     <span>{netTaxPayable >= 0 ? 'صافي الضريبة المستحقة' : 'صافي الضريبة المستردة'}</span>
     <span className={netTaxPayable >= 0 ? 'text-destructive' : 'text-chart-3'}>
