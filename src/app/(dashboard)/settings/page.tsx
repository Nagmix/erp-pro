...
export default function SettingsPage() {
  ...
  const ConnectionBadge = () => {
    if (dataSource === 'erpnext') {
      return (
        <Badge variant="outline" className="gap-1.5 text-xs border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
    ...
    if (dataSource === 'local') {
      return (
        <Badge variant="outline" className="gap-1.5 text-xs border-amber-300 bg-amber-50 text-amber-700 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-300">
          <CloudOff className="h-3 w-3" />محلي (غير متزامن)
        </Badge>
      );
    }
    return (
      <Badge variant="outline" className="gap-1.5 text-xs border-red-300 bg-red-50 text-red-700 dark:border-red-700 dark:bg-red-950 dark:text-red-300">
    ...
    return (
      <div className="flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">تعذر الاتصال بالنظام</p>
          <p className="text-xs text-amber-700 dark:text-amber-300 mt-0.5">{loadError}</p>
        </div>
            ...
            <CardContent>
              ...
              {rolesError && !rolesLoading && (
                <div className="flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950">
                  <AlertTriangle className="h-5 w-5 text-amber-600 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-amber-800 dark:text-amber-200">فشل تحميل الأدوار</p>
                    <p className="text-xs text-amber-700 dark:text-amber-300 mt-0.5">{rolesError}</p>
                  </div>
                  ...
                  {filteredRoles.map((r) => (
                    <div key={r.name} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/30 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${r.disabled ? 'bg-gray-200 text-gray-400 dark:bg-gray-800 dark:text-gray-500' : 'bg-red-500/10 text-red-600'}`}>
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {r.disabled && (
                              <Badge variant="outline" className="text-[9px] h-4 border-gray-300 text-gray-500 dark:border-gray-600 dark:text-gray-400">
                                معطّل
                              </Badge>
                            )}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            {r.desk_access && (
                              <span className="text-[10px] text-emerald-600 dark:text-emerald-400">دخول سطح المكتب</span>
                            )}
                            {r.search_bar && (
                              <span className="text-[10px] text-sky-600 dark:text-sky-400">شريط البحث</span>
                            )}
                            {r.two_factor_auth && (
                              <span className="text-[10px] text-amber-600 dark:text-amber-400">مصادقة ثنائية</span>
                            )}
          ...
          {editingRole && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/35 border border-amber-200 dark:border-amber-800">
                  <div>
                    <p className="text-sm font-medium text-amber-700 dark:text-amber-400">تعطيل الدور</p>
