...
const TRIGGER_EVENTS = [
  ...
  { value: 'debit_note_created', label: 'إنشاء إشعار مدين', icon: FileText, color: 'bg-orange-100 text-orange-700 border-orange-200' },
  { value: 'return_created', label: 'إنشاء مرتجع', icon: FileText, color: 'bg-pink-100 text-pink-700 border-pink-200' },
  { value: 'work_order_completed', label: 'إكمال أمر عمل', icon: CheckCircle2, color: 'bg-amber-100 text-amber-700 border-amber-200' },
] as const;
...
const CHANNEL_OPTIONS = [
  { value: 'email', label: 'بريد إلكتروني', icon: Mail, color: 'bg-sky-100 text-sky-700' },
  { value: 'notification', label: 'إشعار نظام', icon: Activity, color: 'bg-violet-100 text-violet-700' },
  { value: 'both', label: 'بريد + إشعار', icon: Send, color: 'bg-amber-100 text-amber-700' },
] as const;
...
const RECIPIENT_OPTIONS = [
  ...
  { value: 'custom_email', label: 'بريد مخصص', icon: AtSign, color: 'bg-amber-100 text-amber-700' },
] as const;
    ...
    () => [
      ...
      },
      {
        ...
        render: (v) => {
          ...
          return (
            <Badge variant="secondary" className={`text-[10px] ${ch.color}`}>
              {ch.label}
            </Badge>
          );
        },
      ...
      },
      {
        ...
        render: (v) => Number(v) === 1 ? (
          ...
          <span className="text-[10px] text-muted-foreground">لا</span>
        ),
      },
      {
        key: 'last_sent',
        header: 'آخر إرسال',
        render: (v) => v ? (
          <span className="text-[10px] text-muted-foreground">{String(v).slice(0, 16)}</span>
        ) : (
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-lg border border-border/30 bg-background p-3 space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-semibold">٢. القالب والتأخير</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
            ...
            <div className="space-y-4 pb-4">
              ...
              {selectedTemplate && (
                <div className="rounded-lg border border-sky-200/40 bg-sky-50/50 p-2.5 space-y-1.5">
                  ...
                  <p className="text-[10px] text-sky-600 font-medium">{selectedTemplate.subject || 'بدون عنوان'}</p>
              ...
              <div className="space-y-3">
                ...
                {formRecipientType === 'custom_email' && (
                  <div className="space-y-1.5 mt-2">
                    <Label className="text-[10px] text-muted-foreground">عنوان البريد المخصص</Label>
                    <Input
                      type="email"
                      className="h-9"
                      dir="ltr"
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">إرفاق نسخة PDF من المستند</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">تفعيل القاعدة فور الإنشاء</p>
                    </div>
                  </div>
