...
const TRIGGER_EVENTS = [
 ...
 { value: 'appointment_reminder', label: 'تذكير موعد', icon: CalendarDays, color: 'bg-chart-2/10 text-chart-2 border-chart-2/20' },
] as const;
...
const SEND_VIA_OPTIONS = [
 { value: 'sms', label: 'رسالة SMS', icon: Smartphone, color: 'bg-chart-1/10 text-chart-1' },
 { value: 'whatsapp', label: 'واتساب', icon: Phone, color: 'bg-chart-3/10 text-chart-3' },
 { value: 'both', label: 'SMS + واتساب', icon: Radio, color: 'bg-chart-2/10 text-chart-2' },
] as const;
...
const TARGET_OPTIONS = [
 ...
 { value: 'both', label: 'العميل + الموظف', icon: Users, color: 'bg-chart-2/10 text-chart-2' },
] as const;
...
export default function SmsAutoRulesPage() {
 ...
 () => [
  ...
  render: (v) => {
   ...
   return (
   <Badge variant="secondary" className={`text-xs ${via.color}`}>
    {via.label}
   </Badge>
   );
  },
 ...
 <div dir="rtl" className="erp-page-enter space-y-5">
    ...
    <ScrollArea className="max-h-96">
     ...
     <tbody>
      ...
      return (
       ...
       <td className="p-2.5">
        <span className="text-xs">{log.communication_medium || 'SMS'}</span>
       </td>
       <td className="p-2.5">
  ...
  <DialogContent dir="rtl" className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
   ...
   <div className="space-y-4 pb-4">
    ...
    />
    {editingDoc && (
     <p className="text-xs text-muted-foreground">لا يمكن تغيير اسم القاعدة بعد الإنشاء</p>
    )}
    </div>
    ...
    <div className="space-y-3">
     <Label className="text-xs font-semibold flex items-center gap-1.5">
     <Smartphone className="h-3.5 w-3.5 text-chart-1" />
     وسيلة الإرسال
     </Label>
     <div className="space-y-1.5">
    ...
    <div className="flex items-center gap-2">
     ...
     formActive ? 'bg-chart-3/10' : 'bg-muted'
     }`}>
     {formActive ? (
    ...
    <p className="text-xs font-medium text-primary">ملخص القاعدة</p>
    ...
    {editingDoc && (
    <div className="rounded-lg border border-chart-2/20 bg-chart-2/5 p-2.5">
