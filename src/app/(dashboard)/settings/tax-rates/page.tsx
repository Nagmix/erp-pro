...
export default function TaxRatesPage() {
 ...
 <div dir="rtl" className="space-y-6">
  ...
  <Card className="border-border/60 bg-muted/20">
   ...
   <div>
    ...
    {lastSetup.accounts.map((a, i) => (
     ...
     {lastSetup.reused?.accounts?.[i] && (
      <span className="text-[10px] text-chart-2">موجود مسبقاً</span>
     )}
     </li>
    ))}
    </ul>
   ...
   <dl className="grid gap-2 text-xs sm:grid-cols-2">
    ...
    <dd className="font-mono text-[11px] break-all">
     ...
     <span className="text-[10px] text-chart-2">(موجود)</span>
     )}
    </dd>
    </div>
    <div>
  ...
  {!effectiveCompany && (
   <p className="text-xs text-chart-2">اختر شركة لعرض القوالب.</p>
  )}
  ...
  <AlertDialogContent dir="rtl">
   ...
   <AlertDialogAction onClick={runDelete} variant="destructive">
    حذف
   </AlertDialogAction>
   </AlertDialogFooter>
  </AlertDialogContent>
