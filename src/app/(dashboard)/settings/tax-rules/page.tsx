...
const TAX_TYPE_OPTIONS: { value: TaxType; label: string; color: string; description: string }[] = [
  { value: 'vat', label: 'ضريبة القيمة المضافة', color: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300', description: 'ضريبة غير مباشرة على الاستهلاك' },
  { value: 'selective', label: 'ضريبة انتقائية', color: 'bg-amber-500/15 text-amber-700 dark:text-amber-300', description: 'ضريبة على سلع محددة (تبغ، مشروبات…)' },
  { value: 'service', label: 'ضريبة خدمات', color: 'bg-sky-500/15 text-sky-700 dark:text-sky-300', description: 'ضريبة على الخدمات المقدمة' },
  { value: 'withholding', label: 'ضريبة استقطاع', color: 'bg-rose-500/15 text-rose-700 dark:text-rose-300', description: 'ضريبة تُستقطع عند المصدر' },
  { value: 'other', label: 'أخرى', color: 'bg-slate-500/15 text-slate-600 dark:text-slate-300', description: 'نوع ضريبي آخر' },
];
    ...
    () => [
      {
        ...
        render: (_, row) => (
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-[10px] text-muted-foreground font-mono">{row.name}</span>
          </div>
        ),
      },
      {
        ...
        width: 'w-36',
        render: (_, row) => (
          <Badge variant="secondary" className={`text-[10px] font-medium ${taxTypeColor(row.tax_type)}`}>
        ...
        render: (_, row) => (
          <div className="flex items-center gap-1.5">
            {row.included_in_price ? (
              ...
              <span className="flex items-center gap-1 text-[11px] text-orange-700 dark:text-orange-300">
              ...
              <TooltipTrigger asChild>
                ...
                >
                  {row.disabled ? (
                    ...
                    <Badge variant="outline" className="text-[10px] text-emerald-700 border-emerald-400/40 dark:text-emerald-300">
  ...
  const catColumns: Column<TaxCategoryRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_, row) =>
          row.disabled ? (
            <Badge variant="outline" className="text-[10px] text-destructive border-destructive/40">معطّلة</Badge>
          ) : (
            <Badge variant="outline" className="text-[10px] text-emerald-700 border-emerald-400/40 dark:text-emerald-300">مفعّلة</Badge>
          ),
      },
    ],
    []
            ...
            <CardContent className="p-4">
              <div className="flex flex-wrap items-end gap-4">
                ...
                <div className="space-y-1.5 min-w-[160px]">
                  <Label className="text-[10px]">نوع الضريبة</Label>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-4">
                  ...
                  {allRules.length === 0 && !isLoading && (
                    <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40">
                  ...
                  {calcResult ? (
                    <div className="space-y-3">
                      ...
                      {calcResult.includedInPrice && (
                        <div className="flex items-start gap-2 p-2.5 rounded-lg bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-700/30">
