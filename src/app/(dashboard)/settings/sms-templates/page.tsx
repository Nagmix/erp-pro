...
const CATEGORY_OPTIONS = [
  { value: 'invoice', label: 'فاتورة', color: 'bg-sky-100 text-sky-700 border-sky-200' },
  { value: 'quote', label: 'عرض سعر', color: 'bg-violet-100 text-violet-700 border-violet-200' },
  { value: 'credit_note', label: 'إشعار دائن', color: 'bg-emerald-100 text-emerald-700 border-emerald-200' },
  { value: 'debit_note', label: 'إشعار مدين', color: 'bg-rose-100 text-rose-700 border-rose-200' },
  { value: 'reminder', label: 'تذكير', color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { value: 'appointment', label: 'موعد', color: 'bg-teal-100 text-teal-700 border-teal-200' },
  { value: 'general', label: 'عام', color: 'bg-gray-100 text-gray-700 border-gray-200' },
] as const;
        ...
        render: (v, row) => (
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground truncate">{String(row.name)}</p>
            </div>
          </div>
        ),
      },
          ...
          return (
            <div className="max-w-[300px]">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[10px] text-muted-foreground">{info.chars} حرف</span>
                <span className="text-[10px] text-muted-foreground">•</span>
                <span className="text-[10px] font-medium text-primary">{info.parts} رسالة</span>
              </div>
            </div>
          );
        },
          ...
          <DropdownMenuContent align="start">
            ...
            <DropdownMenuItem onClick={() => copyTemplate(row)}>
              {copiedTemplate === row.name ? (
                <CheckCircle2 className="me-2 h-3.5 w-3.5 text-emerald-500" />
              ) : (
          ...
          <div className="flex flex-wrap gap-1.5">
            {TEMPLATE_VARIABLES.map((v) => (
              <button
                key={v.key}
                type="button"
                className="rounded border border-sky-200 bg-sky-50 px-2 py-1 text-[10px] font-mono text-sky-700 hover:bg-sky-100 transition-colors cursor-pointer"
                title={v.description}
                onClick={() => {
              ...
              >
                {v.key}
                <span className="ms-1 text-sky-500 font-sans">({v.label})</span>
              </button>
            ))}
          </div>
        </CardContent>
          ...
          <ScrollArea className="flex-1 -mx-6 px-6">
            <div className="space-y-4 pb-4">
                      ...
                      {DEFAULT_TEMPLATES.map((tpl, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-[10px] font-semibold">{tpl.template_name}</span>
                          </div>
                          <p className="text-[10px] text-muted-foreground line-clamp-2 leading-relaxed" dir="rtl">
                            {tpl.body}
                          </p>
                        </button>
                      ))}
                      ...
                      <PopoverContent dir="rtl" className="w-72 p-2" align="start">
                        <div className="space-y-1">
                          ...
                          {TEMPLATE_VARIABLES.map((v) => (
                            ...
                            >
                              <code className="rounded bg-sky-50 px-1 py-0.5 text-[9px] font-mono text-sky-700 border border-sky-200 shrink-0">
                                {v.key}
                              </code>
                              <div className="min-w-0">
              ...
              <div className="rounded-lg border border-border/50 bg-muted/30 p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-semibold">عدد الأحرف وأجزاء الرسالة</span>
              ...
              <div className="rounded-lg border border-sky-200/40 bg-sky-50/50 p-2.5 space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1">
                  {TEMPLATE_VARIABLES.map((v) => (
                    <button
                      key={v.key}
                      type="button"
                      className="rounded border border-sky-200 bg-white px-1.5 py-0.5 text-[9px] font-mono text-sky-700 hover:bg-sky-100 transition-colors"
                      onClick={() => insertVariable(v.key)}
                    >
                      {v.key}
                    </button>
                ...
                <p className="text-[9px] text-sky-600">اضغط على متغير لإضافته إلى نص الرسالة</p>
              </div>

              {/* Live Preview */}
              {formBody && (
                <div className="rounded-lg border border-emerald-200/40 bg-emerald-50/50 p-3 space-y-1.5">
                  <p className="text-[10px] font-semibold text-emerald-700 flex items-center gap-1">
                    <Eye className="h-3 w-3" />
                    معاينة مباشرة
                  </p>
                  <div className="rounded-md bg-white border border-emerald-200/50 p-2.5">
        ...
        <DialogContent dir="rtl" className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center">
              <div className="w-72 rounded-2xl border-2 border-gray-300 bg-gray-100 p-3 shadow-lg">
                <div className="rounded-xl bg-white p-3 space-y-2">
                  ...
                  <div className="rounded-lg bg-emerald-100 p-2.5">
                    <p className="text-xs leading-relaxed text-emerald-900">{previewText}</p>
                  </div>
            ...
            {/* Original Template */}
            <div className="space-y-1.5">
              <Label className="text-[10px] font-semibold text-muted-foreground">نص القالب الأصلي</Label>
