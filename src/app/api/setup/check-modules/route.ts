...
const MODULE_MAP: Record<string, { label: string; description: string; modules: string[]; requiredApps: string[] }> = {
  ...
  hr: { label: 'الموارد البشرية', description: 'الموظفون، الإجازات، الرواتب، الحضور', modules: ['HR'], requiredApps: ['hrms', 'erpnext'] },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const moduleStatus = Object.entries(MODULE_MAP).map(([id, info]) => {
      ...
      // فحص هل التطبيق المطلوب مثبت
      const appInstalled = info.requiredApps.some((app) =>
